...
class Upsample(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=4, stride=1, padding=1, dropout=False):
        super(Upsample, self).__init__()
        self.dropout = dropout
        self.block = nn.Sequential(
            nn.Upsample(scale_factor=2),
            SelfONN1dlayer(in_channels, out_channels, kernel_size, stride, padding, bias=nn.InstanceNorm1d,q=1),
...
class Downsample(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=4, stride=2, padding=1, apply_instancenorm=False):
        ...
        self.conv = SelfONN1dlayer(in_channels, out_channels, kernel_size, stride, padding, bias=nn.InstanceNorm1d,q=1)
...
class CycleGAN_Unet_Generator(nn.Module):
    def __init__(self, filter=16):
        ...
        self.downsamples = nn.ModuleList([
            Downsample(1, filter, kernel_size=5, padding=2,apply_instancenorm=False),  # (b, filter, 128, 128)
        ...
        self.upsamples = nn.ModuleList([
            Upsample(filter * 8, filter * 8, kernel_size=5,padding=2),
            Upsample(filter * 16, filter * 4, dropout=False, kernel_size=5,padding=2),
            Upsample(filter * 8, filter * 2, dropout=False, kernel_size=5,padding=2),
            Upsample(filter * 4, filter, dropout=False, kernel_size=5,padding=2)
        ])

        self.last = nn.Sequential(
            SelfONNTranspose1dlayer(filter * 2, 1, kernel_size=6, stride=2, padding=2,q=1),
            nn.Tanh()
        )
...
class CycleGAN_Discriminator(nn.Module):
    def __init__(self, filter=16):
        ...
        self.block = nn.Sequential(
            Downsample(1, filter, kernel_size=4, stride=8, apply_instancenorm=False),
            Downsample(filter, filter * 2, kernel_size=4, stride=8),
            Downsample(filter * 2, filter * 4, kernel_size=4, stride=8),
            Downsample(filter * 4, filter * 8, kernel_size=4, stride=4),
            Downsample(filter * 8, filter * 16, kernel_size=4, stride=4),
        )

        self.last = SelfONN1dlayer(filter * 16, 1, kernel_size=3, stride=4, padding=1,q=1)
